...
#define MATCH_DELAY_DURATION 0.2f
#define SCORE_POPUPS_LIFETIME 1.0f
#define SCORE_POPUPS_ALPHA 1.0f
#define SCORE_AMOUNT 10
#define SCORE_SCALE 2.0f
#define SCORE_SCALE_VELOCITY -2.5f
#define MUSIC_VOLUME 0.05f
#define SOUND_VOLUME 0.3f
...
void add_score_popup(int x, int y, int amount, Vector2 grid_origin)
{
    for (int i = 0; i < MAX_SCORE_POPUPS; i++)
    {
        if (!score_popups[i].active)
        {
            ...
            score_popups[i].lifetime = SCORE_POPUPS_LIFETIME;
            score_popups[i].alpha = SCORE_POPUPS_ALPHA;
...
void scoring_actions(int x, int y)
{
    score += SCORE_AMOUNT;
    PlaySound(match_sound);
    score_animating = true;
    score_scale = SCORE_SCALE;
    score_scale_velocity = SCORE_SCALE_VELOCITY;
    add_score_popup(x, y, SCORE_AMOUNT, grid_origin);
}
...
bool find_matched()
{
    ...
    for (int y = 0; y < BOARD_SIZE; y++)
    {
        for (int x = 0; x < BOARD_SIZE - 2; x++) 
        {
            ...
            if (tile == board[y][x + 1] && tile == board[y][x + 2])
            {
    ...
    for (int x = 0; x < BOARD_SIZE; x++)
    {
        for (int y = 0; y < BOARD_SIZE - 2; y++)
        {
            ...
            if (tile == board[y + 1][x] && tile == board[y + 2][x])
            {
...
int main(void)
{
    ...
    SetSoundVolume(match_sound, SOUND_VOLUME);
    ...
    while (!WindowShouldClose())
    {
        ...
        for (int i = 0; i < MAX_SCORE_POPUPS; i++)
        {
            if (score_popups[i].active)
            {
                Color color = Fade(YELLOW, score_popups[i].alpha);
                DrawText(
